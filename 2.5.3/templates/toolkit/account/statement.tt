...
<div class="grd-grid-12 grd-row-padding">
        <div class="grd-grid-2 grd-grid-mobile-6 grd-grid-phablet-6 left-aligned">
                [% IF has_newer %]
                <a class="pjaxload button" href="[% request.url_for('statement.cgi', { after => transactions.first.transaction_time, currency => currency }) %]"><span>&laquo;[% l('Newer') %]</span></a>
                [% END %]
        </div>

        <span class="grd-grid-8 grd-hide-mobile grd-hide-phablet center-aligned" [% IF position != 'top' %] style="visibility:hidden" [% END %]>
            <form action="[% request.url_for('statement.cgi') %]">
        ...
        <div class="grd-grid-2 grd-grid-mobile-6 grd-grid-phablet-6 right-aligned">
                [% IF has_older %]
                <a class="pjaxload button" href="[% request.url_for('statement.cgi', { before => transactions.last.transaction_time, currency => currency }) %]"><span>[% l('Older') %]&raquo;</span></a>
                [% END %]
        </div>
</div>
[%- END -%]
