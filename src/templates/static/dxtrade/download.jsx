...
const Heading = ({ system, children }) => (
    <h1
        ...
        id={`${system}-heading`}
    > {children || it.L(`DXTrade for ${system.replace(/^\w/, c => c.toUpperCase())}`)}
    </h1>
);
...
const DownloadBlock = ({
    ...
    return (
        <div className='gr-4 gr-12-m gr-padding-10' id={`${os}-app`}>
            <img
                className='gr-12 gr-centered dxtrade-mobile-phone'
                src={it.url_for(`images/pages/dxtrade/icons/${/[.]/.test(image) ? image : `${image.svg}`}`)}
            />
            <h3>{title}</h3>
            <p>{desc}</p>
            <DownloadButton
...
const DesktopDownloadBlock = ({
    ...
    >
        <img
            className='gr-12 gr-centered gr-padding-20 dxtrade-desktop-screen-app'
            src={it.url_for(`images/pages/dxtrade/icons/${id}.png`)}
        />
...
const MT5DesktopApp = ({ is_first_child, has_desktop_app }) => {
    const txt_unsupported = it.L('The DXTrade desktop app is not supported by Windows XP, Windows 2003, and Windows Vista.');
    return (
        <div className={`gr-padding-20 desktop-apps${is_first_child ? ' gr-parent' : ''}`}>
            {has_desktop_app ?
                <React.Fragment>
                    ...
                    <p>
                        {it.L(
                            'Download DXTrade for your desktop or laptop to access the powerful tools and features enjoyed by millions of traders.'
                        )}
                    </p>
...
const MT5ForMobile = ({ is_first_child }) => (
    <div className={`gr-padding-30${is_first_child ? ' gr-parent' : ''}`}>
        <h1 className='desktop-apps'>{it.L('DXTrade for Mobile')}</h1>
        ...
        <div className='gr-row' id='mobile-apps'>
            <Heading system='android'>{it.L('DXTrade for Android')}</Heading>
            <Heading system='ios'>{it.L('DXTrade for iOS')}</Heading>
            ...
            <div className='gr-10 gr-push-1 gr-10-p gr-push-1-p gr-12-m gr-push-0-m gr-padding-30'>
                <div className='gr-row browsers'>
                    {['chrome', 'safari', 'firefox', 'opera'].map((browser, idx) => (
                        <img
                            key={idx}
                            className='gr-centered dxtrade-browser-icon'
                            src={it.url_for(`images/pages/dxtrade/icons/${browser}.svg`)}
                        />
                    ))}
                </div>
            </div>
...
const Download = () => (
    <div id='dxtrade_download' className='static_full center-text'>
        <div className='gr-padding-20 gr-child'>
            <h1>{it.L('Start Trading with DXTrade')}</h1>
            <h3>{it.L('Trade with a powerful interface known as the global industry standard.')}</h3>
        </div>
        <div className='invisible' id='dxtrade_download_platforms'>
        ...
        <div className='invisible' id='dxtrade_download_mac_platforms'>
