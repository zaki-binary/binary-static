...
geoXML3.parser = function (options) {
  ...
  var parseKmlString = function (KmlString) {
      ...
      if (window.DOMParser) {
          dom    = new DOMParser();
          KmlXML = dom.parseFromString(KmlString, "text/xml");
      }else {//old IE 
          KmlXML       = new ActiveXObject("Microsoft.XMLDOM");
          KmlXML.async ="false";
          KmlXML.loadXML(KmlString);
      }
      render(KmlXML, thisDoc);
  }
...
function openMap (table_id) {
    initialize()
    $('#popup').fadeIn('fast'); 
}
function loadKML() {
    var availableDatacenter = findHostInDataCenters('#servers_list');
    var dc  = {}
    $.each(availableDatacenter, function(data) { 
        if(data != ''){
            if(dc[data] == null){
                dc[data] = {} 
                dc[data]['info'] = datacentersInfo[data]
                dc[data]['hosts'] = null
            }
            dc[data]['hosts'] = availableDatacenter[data]
       }
    })
    
    var kml = '<?xml version="1.0" encoding="UTF-8"?>'
    kml    += '<kml xmlns="http://www.opengis.net/kml/2.2">'
    $.each(dc, function(dc_name) {
        kml += '<Placemark>'
        kml += '<name>' + dc[dc_name].info.full_name + '</name>'
        kml += '<description><![CDATA[ <div style="text-align:left" dir="ltr"><ul> '
        $.each(dc[dc_name].hosts, function (host){
            kml += '<li>' + dc[dc_name].hosts[host]  + '</li>'
        })
        kml += '</ul></div> ]]></description>'
        kml += '<Point>'
        kml += '<coordinates>' + dc[dc_name].info.longitude + ',' + dc[dc_name].info.latitude + '</coordinates>'
        kml += '</Point>'
        kml += '</Placemark>'
    })
    kml += '</kml>'
...
function findIndexOfColumn(table_id, col_name) {
    var ths = $(table_id).find('thead th')
    var i = 0;
    var result = -1
    $(ths).each(function () {
        if($(this).html() == col_name){
            result = i;
            return false ;
        }
        i += 1;
...
function findDataCenterIndex(table_id){
    return findIndexOfColumn(table_id, 'datacenter')
}
function findNameIndex(table_id){
    return findIndexOfColumn(table_id, 'name')
}

function findHostInDataCenters(table_id) {
    var table_id      = '#servers_list';
    var datacenterCol = findDataCenterIndex(table_id)
    var nameCol       = findNameIndex(table_id)
    var trs           = $(table_id).find('tbody tr')
    var datacenter    = {}
    trs.each(function () {
        var tds  = $(this).find('td')
        var dc   = $(tds.get(datacenterCol)).html()
        var name = $(tds.get(nameCol)).html()
        if(datacenter[dc] == null)
            datacenter[dc] = [name]
        else 
            datacenter[dc].push(name)
    })
...
function initialize() {
    var latlng = new google.maps.LatLng(27.371767, 0.351563);
    var myOptions = {
      zoom: 2,
      center: latlng,
    ...
    var myMap = new google.maps.Map(document.getElementById("map_canvas"),
        myOptions);
    var myParser = new geoXML3.parser({map: myMap, zoom:false});
    myParser.parseKmlString(loadKML())
}
